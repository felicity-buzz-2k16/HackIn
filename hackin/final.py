...
def ques1(request):                # Answer is here
    ...
    if int(person.level) >= 1:
        ...
        return HttpResponseRedirect('/buzz/hackin/levels')
        ...
        if request.method == "GET":
            ...
            if ans == "lyeskpuaitsaamhak":
                if person.level <= 11:
                    ...
                    pass
                return HttpResponseRedirect('/buzz/hackin/ques12')
            else:
                c = {}
                c.update(csrf(request))
                return render_to_response('hackin/ques11.html',c)
...
def ques12(request):
    ...
    if int(person.level) >= 12:
        if request.method == "GET":
            ...
            humptyPasswd = request.POST.get('humptyPasswd','')
            dumptyPasswd = request.POST.get('dumptyPasswd','')
            table_name = request.POST.get('tableName','')
            database = request.POST.get('databaseName','')
            if humptyPasswd == "zombiezone" and dumptyPasswd == "intotheunknown" and table_name == "security" and database == "WHERE_MAGIC_HAPPENS":
                if person.level <= 11:
                    person.level += 1
                    person.save()
                else:
